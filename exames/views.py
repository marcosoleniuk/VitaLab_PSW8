...
def solicitar_exames(request):
    ...
    if request.method == "GET":
        ...
        data_formatada = data_solicitacao.strftime("%d/%m/%Y")
...
def cancelar_pedido(request, pedido_id):
    ...
    if not pedido.usuario == request.user:
        ...
        return render(request, "gerenciar_pedidos/")
...
def permitir_abrir_exame(request, exame_id):
    ...
    if not exame.usuario == request.user:
        ...
        return render(request, "gerenciar_exames/")
    if not exame.requer_senha:
        # verificar se o pdf existe
        if not exame.resultado.name:
            messages.add_message(
                request, constants.ERROR, "Este exame ainda não foi finalizado!"
            )
...
def acesso_medico(request, token):
    ...
    pedidos = (
        PedidosExames.objects.filter(usuario=acesso_medico.usuario)
